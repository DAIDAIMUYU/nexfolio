...
- [!] 修复 Vercel 深层路由直接访问 / 刷新 404。
- [ ] 新增 `vercel.json` rewrite 到 `/index.html`。
- [ ] 复测 `/projects/:id`、`/blog/:id` 直接访问和刷新。
- [!] 补充 SEO 基础文件：favicon、`robots.txt`、`sitemap.xml`。
- [ ] 补充 `index.html` 的 Open Graph / Twitter Card meta。
- [!] 清理占位链接和占位文案。
- [ ] 无真实链接的按钮隐藏、禁用或显示“暂未上线”。
- [!] 修复横向滑动模块鼠标滚轮无法左右滚动的问题。
- [ ] 鼠标位于横向模块内部时，将纵向滚轮映射为横向滚动。
- [ ] 鼠标离开横向模块后，页面恢复正常纵向滚动。
- [ ] 横向滚动到边界后，允许页面继续纵向滚动。
- [ ] 为横向模块增加左右按钮 / 进度条 / 边缘渐隐提示。
- [!] 调整卡片视觉比例，修复椭圆封面嵌套灰色方块的奇怪观感。
- [ ] 统一项目、博客、工具卡片封面比例。
- [ ] 优化 Hero、Footer、社交入口和作品集质感。
- [ ] 复查移动端导航，必要时改为菜单按钮 / 抽屉导航。
- [ ] 更新对应测试用例。
- [ ] 运行 `npm run lint`。
- [ ] 运行 `npm run test`。
- [ ] 运行 `npm run build`。
